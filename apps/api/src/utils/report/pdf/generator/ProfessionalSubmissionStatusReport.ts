...
export class ProfessionalSubmissionStatusReport extends ProfessionalBasePDFGenerator {
  ...
  private addCustomHeader(): void {
    ...
    this.doc.fontSize(12)
      .fillColor('#E2E8F0')
      .font('Helvetica')
      .text('Comprehensive analysis of employee timesheet submission compliance', 0, 65, { 
        align: 'center',
        width: this.pageWidth
      });
  ...
  private addMainDataTable(data: ISubmissionStatusReport[]): void {
    ...
    const headers = ['Employee', 'Email', 'Week Period', 'Status'];
    const columnWidths = [150, 170, 130, 110];
  ...
  private calculateStatistics(data: ISubmissionStatusReport[]) {
    ...
    const missingCount = data.filter(d => d.submissionStatus === 'Missing').length;
    
    // Calculate compliance rate based on records, not unique employees
    const complianceRate = totalRecords > 0 ? 
      Math.round((submittedCount / totalRecords) * 100) : 0;
    
    const totalDaysLate = data.reduce((sum, item) => sum + (item.daysLate || 0), 0);
